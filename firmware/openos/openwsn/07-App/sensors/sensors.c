...
owerror_t sensors_receive(
   OpenQueueEntry_t* msg,
   coap_header_iht*  coap_header,
   coap_option_iht*  coap_options
);
void sensors_sendDone(
   OpenQueueEntry_t* msg,
   owerror_t error
);

const uint8_t sensors_path0[]       = "s";
coap_resource_desc_t sensors_vars;
...
void sensors_init(void) {
   ...
   sensors_vars.path0len            = sizeof(sensors_path0)-1;
   sensors_vars.path0val            = (uint8_t*)(&sensors_path0);
   sensors_vars.path1len            = 0;
   sensors_vars.path1val            = NULL;
   sensors_vars.componentID         = COMPONENT_SENSORS;
   sensors_vars.callbackRx          = &sensors_receive;
   sensors_vars.callbackSendDone    = &sensors_sendDone;

   // register with the CoAP module
   opencoap_register(&sensors_vars);
}
...
*/
void sensors_sendDone(OpenQueueEntry_t* msg, owerror_t error) {
